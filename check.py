...
fig,(ax1, ax2) = plt.subplots(1,2, figsize=(10,4.5), sharey=True, sharex=False)
...
for index, name in enumerate(graphs):
    ...
    if index == 0:
        ...
        ax1.scatter(t1[:,0],t1[:,1],color='grey', label='Upscaled Solutions $\it{s}$=32', marker='.',s=100)
        ax1.scatter(t0[:,0],t0[:,1],color='grey', label='Upscaled Solutions $\it{s}$=16', marker='*',s=100)
        ax1.set_xlim(0,29)
        #ax1.legend()
        ax1.set_title(f'{name}', x=0.2, y=0.5,fontsize=12)
        ...
        ax2.scatter(t1[:,0],t1[:,1],color='grey', label='Upscaled Solutions $\it{s}$=32', marker='.',s=100)
        ax2.scatter(t0[:,0],t0[:,1],color='grey', label='Upscaled Solutions $\it{s}$=16', marker='*',s=100)
        ax2.set_xlim(0,43)
        ax2.legend(fontsize=10)
        ax2.set_title(f'{name}', x=0.2, y=0.5,fontsize=12)
...
plt.savefig('celf_experiments.eps', format='eps')
plt.show()
exit(0)