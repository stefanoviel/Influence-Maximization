...
def LT_model(G, a, p, degree, join, random_generator):
...
def WC_model(G, a, degree, random_generator):                 # a: the set of initial active nodes
...
def MonteCarlo_simulation(G, A, p, no_simulations, model, random_generator=None):
	...
	if model == 'WC':
		degree = {}
		for node in G:
			degree[node] = float(1/G.degree(node))
		for i in range(no_simulations):
			res, time  = WC_model(G, A,degree,random_generator=random_generator)
			times.append(time)
			results.append(res)
	elif model == 'IC':
		for i in range(no_simulations):
		...
		for i in range(no_simulations):
			res, time = LT_model(G, A, p, degree, join,random_generator=random_generator)
			times.append(time)
			results.append(res)

	return (numpy.mean(results), numpy.std(results),sum(times))
