    ...
    if len(population) < 100:
        if sum(args["improvement"]) == 0 and num_generations > 2:
            ...
            for _ in range(new_individuals):
                ...
                args["_ec"].population.append(
                    inspyred.ec.Individual(candidate=candidate))
                args["_ec"].population[-1].fitness = args["fitness_function"](A=candidate)[
                    0]
...
def ea_observer1(population, num_generations, num_evaluations, args):
    ...
    logging.debug("generation {}: individuals diversity {}".format(
        num_generations, ind_div))
...
def ea_observer2(population, num_generations, num_evaluations, args):
    ...
    current_best = max(population).fitness
    if prev_best > 0:
        generation_stats["improvement"] = (
            current_best - prev_best) / prev_best
    else:
...
def hypervolume_observer(population, num_generations, num_evaluations, args):
    ...
    if args["no_obj"] == 3:  # TODO fix
        tot = 100 * ((args["max_seed_nodes"] / args["graph"].number_of_nodes())
                     * 100) * (len(args["communities"])-1)
        ...
        logging.info('Hypervolume at generation {0} : {1} '.format(
            num_generations, current_best))
    elif args["no_obj"] == 2:
        tot = 100 * ((args["max_seed_nodes"] /
                     args["graph"].number_of_nodes()) * 100)
...
def hypervolume_observer_all_combinations(population, num_generations, num_evaluations, args):
    ...
    # access values saved in the nsga2 object
    df = args["nsga2"].df
    ...
    for c in candidates:
        ...
        # extract the list of seed nodes from the dataframe
        df_com_time.loc[len(df_com_time), df_com_time.columns] = elems.iloc[0]

    # In order to have a minimization problem time and #communities must be inverted as well
    df_com_time['time'] = df_com_time['time'].apply(lambda a: -a)
    df_com_time['communities'] = df_com_time['communities'].apply(lambda a: -a)

    # Inverts value for influence and seed set
    for i in range(len(arch)):
        for j in range(len(arch[i])):
            arch[i][j] = -float(arch[i][j])
    F = np.array(arch)

    # computes upper bounds for the hypervolume with different subset of objectives
    tot_influence_seed = 100 * \
        ((args["max_seed_nodes"] / args["graph"].number_of_nodes()) * 100)
    metric_influence_seed = Hypervolume(ref_point=np.array([0, 0]),
                                        norm_ref_point=False,
                                        zero_to_one=False)

    # the highest possible value for node influence is 100 because it represent the percentage of nodes influenced
    # the best value for the objective of time is when time = 1 
    tot_influence_time = 100 * (((max_time - 1) / max_time) * 100)
    metric_influence_time = Hypervolume(ref_point=np.array([0, 0]),
                                        norm_ref_point=False,
                                        zero_to_one=False)

    tot_seed_time = ((args["max_seed_nodes"] / args["graph"].number_of_nodes())
                     * 100) * (((max_time - 1) / max_time) * 100)
    metric_seed_time = Hypervolume(ref_point=np.array([0, 0]),
                                   norm_ref_point=False,
                                   zero_to_one=False)

    tot_influence_seedSize_time = 100 * \
        ((args["max_seed_nodes"] / args["graph"].number_of_nodes())
         * 100) * (((max_time - 1) / max_time) * 100)
    metric_influence_seedSize_time = Hypervolume(ref_point=np.array([0, 0, 0]),
                                                 norm_ref_point=False,
                                                 zero_to_one=False)

    # the highest value for communties is 100 because it represent the percentage of communities influenced
    tot_influence_communities = 100 * 100
    metric_influence_communities = Hypervolume(ref_point=np.array([0, 0]),
                                               norm_ref_point=False,
                                               zero_to_one=False)

    tot_seed_communities = (
        (args["max_seed_nodes"] / args["graph"].number_of_nodes()) * 100) * 100
    metric_seed_communities = Hypervolume(ref_point=np.array([0, 0]),
                                          norm_ref_point=False,
                                          zero_to_one=False)

    tot_influence_seedSize_communities = 100 * \
        ((args["max_seed_nodes"] / args["graph"].number_of_nodes())
         * 100) * 100
    metric_influence_seedSize_communities = Hypervolume(ref_point=np.array([0, 0, 0]),
                                                        norm_ref_point=False,
                                                        zero_to_one=False)

    tot_influence_seedSize_communities_time = 100 * ((args["max_seed_nodes"] / args["graph"].number_of_nodes()) * 100) * 100 * (
        ((max_time - 1) / max_time) * 100)  	
    metric_influence_seedSize_communities_time = Hypervolume(ref_point=np.array([0, 0, 0, 0]),
                                                             norm_ref_point=False,
                                                             zero_to_one=False)
    ...
    if args["elements_objective_function"] == "influence_seedSize":
        ...
        hv_influence_seedSize_time = metric_influence_seedSize_time(
            np.concatenate([F, np.transpose([df_com_time['time']])], axis=1))
        ...
        hv_influence_seedSize_communities = metric_influence_seedSize_communities(
            np.concatenate([F, np.transpose([df_com_time['communities']])], axis=1))
        hv_influence_seedSize_communities = hv_influence_seedSize_communities / \
            tot_influence_seedSize_communities
        # print("\thv_influence_seedSize_communities:", hv_influence_seedSize_communities)

        args["hypervolume"].append(
            [hv_influence_seed, hv_influence_seedSize_time, hv_influence_seedSize_communities])
        ...
        hv_influence_seedSize_communities = metric_influence_seedSize_communities(
            np.concatenate([F[:, :-1], np.transpose([df_com_time['communities']])], axis=1))
        hv_influence_seedSize_communities = hv_influence_seedSize_communities / \
            tot_influence_seedSize_communities
        # print("\thv_influence_seedSize_communities:", hv_influence_seedSize_communities)

        hv_influence_time = metric_influence_time(np.concatenate(
            [np.transpose([F[:, 0]]), np.transpose([F[:, 2]])], axis=1))
        ...
        args["hypervolume"].append([hv_influence_seed, hv_influence_seedSize_time,
                                   hv_influence_seedSize_communities, hv_influence_time, hv_seed_time])
        ...
        hv_influence_seedSize_time = metric_influence_seedSize_time(
            np.concatenate([F[:, :-1], np.transpose([df_com_time['time']])], axis=1))
        hv_influence_seedSize_time = hv_influence_seedSize_time/tot_influence_seedSize_time
        # print("\thv_influence_seedSize_time:", hv_influence_seedSize_time)

        hv_influence_communities = metric_influence_communities(
            np.concatenate([np.transpose([F[:, 0]]), np.transpose([F[:, 2]])], axis=1))
        ...
        hv_influence_seedSize_communities_time = metric_influence_seedSize_communities_time(
            F)
        hv_influence_seedSize_communities_time = hv_influence_seedSize_communities_time / \
            tot_influence_seedSize_communities_time
        ...
        hv_influence_time = metric_influence_time(np.concatenate(
            [np.transpose([F[:, 0]]), np.transpose([F[:, 3]])], axis=1))
        hv_influence_time = hv_influence_time/tot_influence_time
        # print("\thv_influence_time:", hv_influence_time)

        hv_seed_time = metric_seed_time(np.concatenate(
            [np.transpose([F[:, 1]]), np.transpose([F[:, 3]])], axis=1))
        hv_seed_time = hv_seed_time/tot_seed_time
        # print("\thv_seed_time:", hv_seed_time)
        ...
        hv_influence_seedSize_time = metric_influence_seedSize_time(
            np.concatenate([F[:, :2], np.transpose([F[:, 3]])], axis=1))
        hv_influence_seedSize_time = hv_influence_seedSize_time/tot_influence_seedSize_time
        # print("\thv_influence_seedSize_time:", hv_influence_seedSize_time)
        ...
        hv_influence_time = metric_influence_time(
            np.concatenate([F[:, :1], np.transpose([F[:, 2]])], axis=1))
        hv_influence_time = hv_influence_time/tot_influence_time
        ...
        args["hypervolume"].append(
            [hv_influence_time, hv_influence_communities])
