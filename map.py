...
for graph in graph_list:
    for s in scale_list:
        ...
        for measure in degree_measure:
            MAP_RESULTS[measure] = []
        filename = "scale_graphs/{0}_{1}.txt".format(graph, s)
        scale_comm = "comm_ground_truth/{0}_{1}.csv".format(graph, s)
        ...
        df_scale_results = pd.read_csv("experiments/{0}_{1}-LT/run-1.csv".format(graph,s),sep=",")
        df_scale_results = df_scale_results.sort_values(by="n_nodes", ascending=False)
        nodes = df_scale_results["nodes"].to_list()

        filename_original_results = "experiments/{0}-LT/run-1.csv".format(graph)
        ...
        for measure in degree_measure:
            ...
            df_mapping.to_csv('{0}_LT_{1}-{2}.csv'.format(graph,s,measure), index=False)
        ...
        df_final["GD"] = gd
        df_final.to_csv('{0}_LT_{1}_MAPPING.csv'.format(graph,s), index=False)
        exit(0)
