...
if hbar: 
    for db, name in  zip([facebook_com, fb_pol, pgp, deezer], ['facebook_com',  'fb_pol', 'pgp', 'deezer']): 
        ...
        for dimensions in ['influence seed communities', 'influence seed', 'influence seed time']: 
            # db['HV values'] = - np.log(db['HV values'])
            sns.catplot(data=db.loc[db['HV dimensions'] == dimensions ], x='IC probability', y='HV values', kind="bar", hue="fitness_function",
                        legend=False, height=5, aspect=6/5,  palette= sns.color_palette("deep"))
    ...
    for db, name in  zip([facebook_com, fb_pol, pgp, deezer], ['facebook_com',  'fb_pol', 'pgp', 'deezer']): 
        ...
        print(name)
        df = db.loc[db['HV dimensions'] != 'influence seed']
