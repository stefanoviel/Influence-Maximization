...
if __name__ == '__main__':
    ...
    for measure in degree_measure:
        ...
        df_mapping.to_csv('{0}_{1}_{2}-{3}.csv'.format(args["graph"],args["model"],args["s"],measure), index=False)
    ...
    df_final["GD"] = gd
    df_final.to_csv('{0}_{1}_{2}_MAPPING.csv'.format(args["graph"],args["model"],args["s"]), index=False)
