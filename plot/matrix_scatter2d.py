...
data_labels_1 = []
data_to_plot_1 = []
with open('/Users/elia/Desktop/final_experiments/amazon0302/amazon0302-k200-WC.csv', 'rU') as csvfile:
    import pandas as pd
    reader = pd.read_csv(csvfile, delimiter=',', quotechar='|')
    row = []
    row.append(reader["n_nodes"])
    row.append(reader["influence"])
    row.append(reader["n_simulation"])
    labels = ["n_nodes", "influence", "n_simulation"]
    for i in range(len(row)):
        data_labels_1.append(labels[i])
        data_to_plot_1.append(row[i])

fig=matrixScatter2D(data_to_plot_1, data_labels_1, False)
fig.savefig('metrics_gp.png', format='png', dpi=100)

data_labels_2 = []
data_to_plot_2 = []
with open('/Users/elia/Desktop/final_experiments/ego-facebook/Facebook-k200-0.1-IC.csv', 'rU') as csvfile:
    import pandas as pd
    reader = pd.read_csv(csvfile, delimiter=',', quotechar='|')
    row = []
    row.append(reader["n_nodes"])
    ...
    labels = ["n_nodes", "influence", "n_simulation"]
    for i in range(len(row)):
        data_labels_2.append(labels[i])
        data_to_plot_2.append(row[i])

fig=matrixScatter2D(data_to_plot_2, data_labels_2, False)
fig.savefig('metrics_baseline.png', format='png', dpi=100)

fig=matrixScatter2D_2(data_to_plot_2, data_to_plot_1, data_labels_2, False)
fig.savefig('metrics.png', format='png', dpi=100)
